...
//en el on le digo lo que voy a transmitir, le paso el topic
...
/////////////////////////////////////////////////////////////////////////////
///26/01/21
/*	//	Principios de Inversión de la dependencias		*///

//Recordar: Los eventos no se pueden realizar entre hermanos
...
DI:	Inyector de Dependencias
	...
	//usar el service locator es usar un antipatron
	//yo estoy localizando el servicio pubsub que lo va a ejecutar un antipatron
	
//esto también se encuentra en la documentacion de customEvent MDN
class Foo {
	constructor(A){}
}

1. Cada hijo solicita al primer ancestro que tenga el PubSub las
instancias creadas por el ancestro.

//Para conectar las cajas nos hace falta
...
//Un wrapper es una funcion que envuelve a otra funcion
//En nuestro calendario, necesitamos cambiar todo lo que sea dependencia del DOM
//por todo lo que hay en el wrapper.

//Todo en el DOM - Funciones
export function create(tag,namespace="http://www.w3.org/1999/xhtml"){
    return document.createElementNS(namespace,tag)
}
export function append(node,child,sibling=null){
    node.insertBefore(child,sibling)
}
export function remove(node){
    node.prentNode.removeChild(node)
}
export function event(node,handler,useCapture=false){
    node.addEventListener(handler,useCapture)
    return ()=>node.removeEventListener(handler,useCapture)
}
export function addClasess(node,...classes){
    classes.forEach(c=>{
        node.classList.add(c)
    })
}
export function removeClasess(node,...classes){
    classes.forEach(c=>{
...
/*	Tareas a Realizar en el Calendario	*/
/*	Vamos a comenzar por las tareas independientes hasta llegar a las Dependientes	
	Nunca Empezar a construir el Padre, hasta no tener construidos todos los hijos	*/
0. Crear CSS
1. Crear en todos los componentes un shadow
2. Crear una hoja stylo unica para todas las instancias del componente
3. Wrappear todo el dom
4. Hacer que los botones funciones
	up
        down publicador suscriptor
5. Crear Dias de la semana culture, un array para el ingles y el español.

//	Características de Clases y Objetos:
Objeto = estado.
-	Como quieres mantener el estado
-	Hasta que punto quieres protegerlo #
variables --> publicas
			  privadas
Clases
Definir la plantilla de un Objeto
atributos
	constructor
	setter
	getter
...
El shadow dom tiene la intencion de hacer privado, el html y el css de un componente.
Metodo que se usa para encapsular elementos html y css, para que no se vean de cara al usuario.
_ = {mode: open} HTML/CSS
# = {mode: close} JavaScript

El uso del Shadow DOM hace que el código sea más compacto, ahorra trabajo de lectura 
a los programadores. Por ejemplo, si existen elementos css y tienen clases, podemos
jugar con el shadow DOM, incluyendo en el, esas clases del elemento. Pero lo importante
es quedarse con el concepto del uso del Shadow, más allá de otros matices.

Clausure: puedo encerrar y guardar el estado de una función, con valores determinados.

//		CODIGO ASÍNCRONO	90% del codigo actual ////
...
//Asíncrono:	
		...
		event,
		promise,
Necesito un callback para recoger los datos:
Por eso necesito un callback, pasar la funcion como parametro a otra funcion.
// en algun sitio necesita darme el resultado en una funcion, me informará con una función o callback
//Necesito pasarle una funcion para que, CUANDO EL PUEDA, me pase el resultado con los valore esperados.

/*
1.	callback(flecha de la muerte)	//incorrecto
2.	Promise - resolve-->then(n)		reject-->catch(1)
	Si tengo datos, te prometo que algo te voy a dar, 
	lanza algo seguro, datos o error.
3.	async await
4.	Fetch API - pasamos los datos por JSON
				response = await fetch('customer.json')
...
////////	ejemplo: cuidao con la flecha de la muerte! 	//////
	class ServiceCustomer{
		get(id,cb){
			cb({id:1,phone:3})
		}
	}
	class ServiceInvoices{ //una vez que tengo los datos del cliente, recupero su factura
		get(customerId,cb){
			cb({id:1,customer_id:3})
		}
	}
	class Program{
		constructor(serviceCustomer,serviceInvoices){
			this.serviceCustomer = serviceCustomer;
			this.serviceInvoices = servicesInvoices;
		}
		//una funcion que espera algo asincrono para devolverme algo
		//la flecha de la muerte/el callback lo hace mas costoso
		//anidar callbacks es una mala práctica
		run(id){
			this.serviceCustomer(id,(customerId)=>{
				this.serviceInvoices(customerId,(invoices)=>{
...
//////////////////		Convertimos nuestro codigo a promise	//////////////////
//
	class ServiceCustomer{
    get(id){
        return new Promise((resolve,reject)=>{
            if(id===1){
                resolve({id:id,phone:3})
            }
            else{
	 ...
	 class ServiceInvoices{
		...
		return new Promise((resolve,reject)=>{
			if(customerId===1){
				resolve({id:1,phone:3,customerId})
			}
			else{
				reject("El cliente no existe")
			}
	...
	//Ahora creo una clase para el programa
	class Program{
		constructor(serviceCustomer,serviceInvoices){
			this.serviceCustomer = serviceCustomer;
			this.serviceInvoices = servicesInvoices;
		}
		//todo lo que se resuelve lo voy cogiendo por then
		run(id){
			//debugger;
			//start
			console.log("start") //sincrono (1)
			this.serviceCustomer.get(id) //asincrono
			.then((data)=>{
				return data.id
			})
			.then((customerId)=>{
				return this.serviceInvoices.get(customerId)
			})
			.then((invoice)=>{
...
//Vease el articulo, interfaz fluida
https://es.wikipedia.org/wiki/Interfaz_fluida
//Se consigue la interfaz fluida
...
class Program{
		...
		async run(customerId){
			//debugger;
			try{
				const{id} = await this.serviceCustomer.get(customerId)
				const invoice = await this.serviceInvoices.get(id);
				console.log(invoice);
			}
			catch(err){
				console.log(err);
...
//JSON	-	Primero espero la respuesta con el await y luego la paso a objeto
Un JSON es como una tubería por la que se pasan los datos en forma de cadena string
los archivos se transmiten al servidor de manera asincrona
fetch("customer.json")
...
Revisar Librerias para fechas
momment
datefns
dayjs

Revisar Librerias para PubSub
Redux
lit-element//pero si usamos esta, no practicamos los webcomponents
...
Modelo Objetos
	...
	estado, constructor, destruye
	metodo
	get, set(azucar sintactico sobre metodo)Establecer - Modificar
	Herencia y Mixing
	array.map
		 .reduce
		 .sort
...
html
	...
	Ciclo de vida:
		cuando me creo
		cuando heredo del padre
		cuando me destruyo
	Eventos
CSS
	selectores
	reglas de preferencia //cuando se habilita una cosa y cuando no
	modelo de caja
	display
	position
	grid
	flexbox
//////	Planteamiento y Organizacion de un Proyecto 	//////////////////
Maestro de Orquesta 	TimerService setInterval
								customerService->servidor
								pubsub-->topics
								cultura
								config
	calendar
			timer
				date ---> topics ---> pubsub //le llega la info y punto.
				formatDate(){"HH:mm:ss"}
			system-date
				date ---> topics ---> pubsub
				formatDate(){"Full"}
			...
			daysofWeekend
					["L","M","X","J","V","S","D"]
			gridMonth
				date --->{date,boolean,boolean} --> DateService
				[gridDay]
				
	//puedo crear los dias con una arrow-function	
		["L","M","X","J","V","S","D"].map(d=>{
			let div = document.createElement('div');
			div.textContent = d;
			return div;
		}).forEach(d=>this.appendChild(d))

/*	Coge la Pagina de Google y haz una descomposicion de ella	*/

//Nueva Pestaña de Google:
//	planteamiento
localizar el origen de los datos
como son los datos que voy a pintar
ver si existe algún pubsub, porque si lo hay, debemos tener en cuenta sus dependencias.

serviceGoogleHome
//En estos casos NO habría Pubsub
pubsub x 
//si los padres no tienen nada que decirle a los hijos
//si los hermanos no tiene nada que decirse entre ellos
